...
// Load current tribe count from server
async function loadTribeCount() {
    try {
        const response = await fetch(`${API_BASE_URL}/api/tribe-count`);
        ...
        console.error('Error loading tribe count:', error);
...
async function checkIfFollower(email) {
    try {
        const response = await fetch(`${API_BASE_URL}/api/check-follower`, {
            method: 'POST',
            headers: {
...
// Add new follower to server
async function addFollowerToServer(email) {
    try {
        const response = await fetch(`${API_BASE_URL}/api/add-follower`, {
            method: 'POST',
            headers: {
                'Content-Type': 'application/json',
            },
...
verifyBtn.addEventListener('click', async function() {
    ...
    try {
        // Check if email is already a follower
        const isAlreadyFollower = await checkIfFollower(email);
        ...
        // Email is valid and not a follower
        setTimeout(() => {
...
followForm.addEventListener('submit', async function(e) {
    ...
    try {
        // Add follower to server
        const result = await addFollowerToServer(email);
...
contactForm.addEventListener('submit', async function(e) {
    ...
    try {
        ...
        const result = await sendContactToServer(name, email, message);
...
document.addEventListener('DOMContentLoaded', () => {
    // Load real-time tribe count from server
    loadTribeCount();
});
